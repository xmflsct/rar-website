...
const Bag = () => {
  ...
  const excludeDates = []
  for (let i = 0; i < 31; i++) {
    const weekday = new Date(2022, 5, i).getDay()
    if (weekday === 1 || weekday === 2) {
      excludeDates.push(new Date(2022, 5, i))
    }
  }
  for (let i = 0; i < 32; i++) {
    const weekday = new Date(2022, 6, i).getDay()
    if (weekday === 1 || weekday === 2) {
      excludeDates.push(new Date(2022, 6, i))
    }
  }
